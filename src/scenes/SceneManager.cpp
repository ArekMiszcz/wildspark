...
void SceneManager::addScene(SceneType type, std::unique_ptr<Scene> scene) {
    ...
    if (!scene) {
        std::cerr << "SceneManager::addScene: ERROR - Attempted to add null scene for type " << static_cast<int>(type) << std::endl;
        return;
    }
    ...
    if (it != scenes.end()) {
        ...
        std::cerr << "SceneManager::addScene: VERIFICATION FAILED: Scene type " << static_cast<int>(type) << " NOT found in map immediately after adding." << std::endl;
    }
}
...
void SceneManager::removeScene(SceneType type) {
    ...
    auto it = scenes.find(type);
    if (it == scenes.end()) {
        std::cerr << "SceneManager::removeScene: Scene type " << static_cast<int>(type) << " not found in map." << std::endl;
        return;
    }
...
void SceneManager::processSceneSwitch() {
    if (requestedSceneType != SceneType::None && requestedSceneType != currentSceneType) {
        ...
        if (it != scenes.end()) {
            ...
            std::cerr << "SceneManager::processSceneSwitch: ERROR - Scene type " << static_cast<int>(requestedSceneType) << " NOT found in map." << std::endl;
            std::cerr << "SceneManager::processSceneSwitch: Current map contents (size " << scenes.size() << "):" << std::endl;
            for(const auto& pair : scenes) {
                std::cerr << "SceneManager::processSceneSwitch: Map has key: " << static_cast<int>(pair.first) << " -> points to scene: " << (pair.second ? "valid" : "invalid/null") << std::endl;
            }
            // Fallback if scene not found
            currentScene = nullptr;
            currentSceneType = SceneType::None; // This is why it reverts to None if find fails
            std::cerr << "SceneManager::processSceneSwitch: Reset currentScene to nullptr and currentSceneType to None due to find failure." << std::endl;
        }
