...
TEST_F(CharacterCreationSceneTest, HandleEventProcessesEvent) {
    characterCreationScene->onEnter(*mockSceneManager);
    sf::Event testEvent = sf::Event::KeyPressed{sf::Keyboard::Key::A, sf::Keyboard::Scan::A, false, false, false, false};
    EXPECT_NO_THROW(characterCreationScene->handleEvent(testEvent, *mockSceneManager));
    characterCreationScene->onExit(*mockSceneManager);
}

TEST_F(CharacterCreationSceneTest, UpdatePerformsTick) {
    characterCreationScene->onEnter(*mockSceneManager);
    EXPECT_NO_THROW(characterCreationScene->update(sf::seconds(0.016f), *mockSceneManager));
    characterCreationScene->onExit(*mockSceneManager);
}

TEST_F(CharacterCreationSceneTest, RenderDrawsScene) {
    characterCreationScene->onEnter(*mockSceneManager);
    ImGui::SFML::Update(mockWindow, sf::seconds(1.f / 60.f)); // Update ImGui-SFML state
...
TEST_F(CharacterCreationSceneTest, SaveCharacterActionCallsAccountManagerAndSwitchesSceneOnSuccess) {
    characterCreationScene->onEnter(*mockSceneManager);

    SetInitialCharacterFormState("ValidName", 0);

    EXPECT_CALL(*mockAccountManager, saveCharacter(::testing::StrEq("ValidName"), ::testing::StrEq("Male"), ::testing::_, ::testing::_))
        .WillOnce(::testing::Invoke([](const std::string& name,
...
TEST_F(CharacterCreationSceneTest, SaveCharacterActionHandlesFailure) {
    ...
    EXPECT_CALL(*mockAccountManager, saveCharacter(::testing::StrEq("AnotherName"), ::testing::StrEq("Male"), ::testing::_, ::testing::_))
        .WillOnce(::testing::Invoke([](const std::string& name,
                                         const std::string& sex,
                                         std::function<void(const Nakama::NStorageObjectAcks&)> successCb,
                                         std::function<void(const Nakama::NError&)> errorCb) {
...
TEST_F(CharacterCreationSceneTest, SaveCharacterActionWithEmptyNameDoesNotCallManager) {
    characterCreationScene->onEnter(*mockSceneManager); // This should clear characterName

    ASSERT_EQ(GetCharacterNameFirstChar(), '\0');

    EXPECT_CALL(*mockAccountManager, saveCharacter(::testing::_, ::testing::_, ::testing::_, ::testing::_))
        .Times(0);
    EXPECT_CALL(*mockSceneManager, switchTo(::testing::_))
        .Times(0);
...
TEST_F(CharacterCreationSceneTest, SaveCharacterActionUsesFormInputValues) {
    ...
    EXPECT_CALL(*mockAccountManager, saveCharacter(::testing::StrEq(testName), ::testing::StrEq("Female"), ::testing::_, ::testing::_))
        .WillOnce(::testing::Invoke([](const std::string& name,
                                         const std::string& sex,
                                         std::function<void(const Nakama::NStorageObjectAcks&)> successCb,
                                         std::function<void(const Nakama::NError&)> errorCb) {
