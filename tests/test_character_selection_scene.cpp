...
TEST_F(CharacterSelectionSceneTest, OnEnterSetsUpSceneAndListsCharacters) {
    EXPECT_CALL(*mockAccountManager, listCharacters(::testing::_, ::testing::_)).Times(1);
    characterSelectionScene->onEnter(*mockSceneManager);
    characterSelectionScene->onExit(*mockSceneManager);
}
...
TEST_F(CharacterSelectionSceneTest, HandleEventProcessesEvent) {
    ...
    EXPECT_NO_THROW(characterSelectionScene->handleEvent(testEvent, *mockSceneManager));
    characterSelectionScene->onExit(*mockSceneManager);
}

TEST_F(CharacterSelectionSceneTest, UpdatePerformsTick) {
    characterSelectionScene->onEnter(*mockSceneManager);
    EXPECT_NO_THROW(characterSelectionScene->update(sf::seconds(0.016f), *mockSceneManager));
    characterSelectionScene->onExit(*mockSceneManager);
}
...
TEST_F(CharacterSelectionSceneTest, RenderDrawsScene) {
    ...
    ASSERT_NE(ImGui::GetCurrentContext(), nullptr) << "ImGui context is null after ImGui::SFML::Update.";
...
TEST_F(CharacterSelectionSceneTest, CreateCharacterActionSwitchesToCharacterCreationScene) {
    characterSelectionScene->onEnter(*mockSceneManager);
    EXPECT_CALL(*mockSceneManager, switchTo(SceneType::CharacterCreation)).Times(1);
...
TEST_F(CharacterSelectionSceneTest, ListCharactersSuccessUpdatesStatus) {
    EXPECT_CALL(*mockAccountManager, listCharacters(::testing::_, ::testing::_))
        .WillOnce(::testing::Invoke([](std::function<void(Nakama::NStorageObjectListPtr)> successCb, 
                                         std::function<void(const Nakama::NError&)> errorCb) {
            Nakama::NStorageObjectListPtr fakeList = std::make_shared<Nakama::NStorageObjectList>();
            successCb(fakeList);
        }));
