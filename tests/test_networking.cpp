...
TEST(NetworkingTest, JoinMatchWhenNotConnected) {
    ...
    // Expect connect to be called on the rtClient (protocol defaulted internally). Accept any protocol enum.
    EXPECT_CALL(*mockRtClient, connect(testing::_, testing::Eq(true), testing::_)).Times(1);
...
TEST(NetworkingTest, SendPlayerUpdate) {
    ...
    EXPECT_CALL(*mockRtClient, isConnected()).Times(testing::AtLeast(1)).WillRepeatedly(testing::Return(true));
...
TEST(NetworkingTest, SendPlayerUpdateNotConnected) {
    ...
    EXPECT_CALL(*mockRtClient, isConnected()).Times(testing::AtLeast(1)).WillRepeatedly(testing::Return(false));
